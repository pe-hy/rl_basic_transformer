...
graphs = add_enabling(gen_directed_graphs(n=GRAPHS_NUM, m=NODES_NUM))
...
error_samples = []
logs = []
save_interval = 100  # Save every 100 successful samples
...
for i in tqdm(indexes):
    for sample in get_sample(vecs[i], graphs):
        try:
            ...
            if states is not None:
                logs.append({'sample': sample, 'target_nodes': target_nodes, 'log': sample_log})
                
                # Save every save_interval successful samples
                if len(logs) % save_interval == 0:
                    with open(f'progress_{len(logs)}.pkl', 'wb') as f:
                        pickle.dump({'total': total, 'error_samples': error_samples, 'logs': logs}, f)
                    print(f"Saved progress at {len(logs)} samples")
