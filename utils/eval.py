...
from utils.countdown_bfs import bfs
from utils.countdown_dfs import dfs
...
def eval_ll(
    model, tokenizer, data, batch_size=128, context_len=4096, temperature=0.0, n=1
):
    """
    ...
    for b in tqdm.trange(0, len(data), batch_size):
        ...
        if n == 1:
            ...
            for i in range(n):
                ...
                ratings = [
                    metric_fn(ot.split(tokenizer.bos_token)[1], mode="sft")[0]
            ...
            output_texts = [
                all_outputs[max_r][i] for i, max_r in enumerate(max_ratings)
            ]
            output_texts_concat += output_texts
...
results_file = os.path.join(
    ckpt_dir, f"results_{args.data.replace('/','_')}_{args.num}_{args.offset}"
)
