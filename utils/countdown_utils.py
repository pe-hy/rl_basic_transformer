...
import re
import json
from utils.countdown_validator import *
...
def parse_trajectory(search_path, mode="dt"):
    ...
    if not target_nums_match:
        ret = "Invalid input: Cannot find the initial state in the first line."
        return ret
    ...
    if not goal_lines:
        ret = "No goal reached statement found."
        return ret
    ...
    operations = re.findall(
        r"E (\d+\s*.\s*\d+\s*=\s*\d+)", search_path[: goal_line.start()]
    )
    if not operations:
        ret = "No operations found leading to the goal."
        return ret
...
def metric_fn(search_path, mode="dt"):
    ...
    if rating == "Valid path.":
        ...
        # if "->" in first_line:
        #     first_line = first_line.split("->")[1]
        target_nums_match = re.match(
            r"S\s+(\d+)\s+\[\s+(\d+(?:\s+\d+)*)\s+\]", first_line
        )
        # target_nums_match = re.match(r"Current State: (\d+):\[(.*?)\]", first_line) TODO
        target, nums = int(target_nums_match.group(1)), [
            int(n) for n in target_nums_match.group(2).split(" ")
